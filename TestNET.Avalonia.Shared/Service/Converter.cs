...
namespace TestNET.Avalonia.Shared.Service;

public class IndexConverter : IValueConverter
{
    public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
    {
        if (value is Control item)
        {
...
public class SubmissionAttentionConverter : IMultiValueConverter
{
    public object? Convert(IList<object?> values, Type targetType, object? parameter, CultureInfo culture)
    {
        ...
        {
            var requiresAttention = Application.Current.FindResource("requiresAttention");
...
public class PointsOutOfMaxConverter : IMultiValueConverter
{
    public object? Convert(IList<object?> values, Type targetType, object? parameter, CultureInfo culture)
    {
        if (values.Count == 2 && values[0] is float points && values[1] is float max)
            return $@"{points}/{max}";
        return "0";
    }
